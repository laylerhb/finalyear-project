...
class TeacherPage extends Component {
  ...
  fileSelectorHandler = (event, type) => {
    console.log("file selected", event.target.files);
    const selectedfile = { ...this.state.Form };
    selectedfile[type].value = event.target.files[0];

    // For images, we create a preview URL; for PDFs, we just store the file name
    if (type === "image") {
        selectedfile[type].name = URL.createObjectURL(event.target.files[0]);
    } else {
        selectedfile[type].name = event.target.files[0].name;
    }
  ...
  sumbitButton = () => {
    ...
    console.log("form data ready to submit", fd);
    ...
    if (this.OverallValidity()) {
      axios
        ...
        .then((res) => {
          ...
          this.setState({ CourseId: res.data.newCourse.id });
    ...
    return (
      <Layout>
        <div className="container-fluid-main">
          ...
          <div className='cloudpng'>Upload your content</div>
          <div className="Teacher-Head-Class">
            <label className="custom-image-upload">
              <input
                ...
                name="image"
                accept="image/*"
                onChange={(event) => this.fileSelectorHandler(event, "image")}
              />
              Upload Image
            </label>

            <p className="ImageName">{fileName || "No file selected"}</p>
            <img
              className=""
          ...
          <div className="Teacher-Head-Class">
            <label className="custom-image-upload">
              <input
                ...
                onChange={(event) => this.fileSelectorHandler(event, "pdf")}
              />
              Upload PDF
            </label>
            <p className="ImageName">{this.state.Form.pdf.name || "No file selected"}</p>
          </div>
