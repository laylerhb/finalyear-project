...
class CoursePage extends Component {
  ...
  componentDidMount = async () => {
    const response = await AuthServices.FetchCourses(this.state.CourseType, this.state.CourseId)

    if(!response){
      throw Error('Failed to fetch course data');
    }

    console.log("CoursePage Response", response.data.course);

    this.setState({
      CoursesInfo: response.data.course,
      CurrentVideo: response.data.course.videos[0],
      loading: false,
    });
    ...
    for (let j in response.data.course.videos) {
      for (let i in response.data.course.videos[j].usersWatched) {
        if (
          localStorage.getItem("userId") ===
          response.data.course.videos[j].usersWatched[i]
        ) {
    ...
    let progress = (count / this.state.CoursesInfo.videos.length) * 100;
  ...
  render() {
    ...
    if (this.state.loading === false) {
      ...
      ratingtimesUpdated = this.state.CoursesInfo.timesUpdated;
      videourl = this.state.CoursesInfo.videos[0]
        ? this.state.CoursesInfo.videos.map((video) => video.videoUrl)
        : [];
      resourcePath = this.state.CoursesInfo.resourcePath;
      ...
      <Layout>
        <div className="coursePage">
          <div className="container">
            ...
            <div>
              {this.state.CoursesInfo?.resourcePath && (
                <div
                ...
                >
                  ...
                  <iframe
                    src={Url + this.state.CoursesInfo.resourcePath}
                    width="100%"
                    height="500px"
                    title="Course PDF"
                    style={{
                  ...
                  <div style={{ marginTop: "10px" }}>
                    <a
                      href={Url + this.state.CoursesInfo.resourcePath}
                      target="_blank"
